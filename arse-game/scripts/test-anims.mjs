...
// ---- data sanity ----
{
  ...
  check("expected clips present", ["idle", "walk", "run", "sprint", "hover", "punch", "punchM", "cross", "uppercut", "uppercutM", "kick", "hit", "cast1", "cast2", "throw", "bash", "snatch", "land", "block", "fightIdle", "punchFlurry", "punchFlurryM", "hookFlurry"].every((k) => ANIMS.clips[k]));
...
}
{
  ...
  let bad = [];
  for (const name of ["punch", "punchM", "cross", "uppercut", "uppercutM", "kick", "cast1", "cast2", "throw", "bash", "snatch", "land", "hit", "block", "fightIdle", "idle", "hover", "punchFlurry", "punchFlurryM", "hookFlurry"]) {
    const c = ANIMS.clips[name];
    // strike/grapple clips have authentic elbow-lead (throw), wrap (snatch), elbow-strikes (uppercut)
    const tol = (name === "uppercut" || name === "uppercutM") ? 0.92 : name === "snatch" ? 0.8 : name === "throw" ? 0.6 : 0.35;
