...
async function bakeClip(name, file, trim, rootPolicy, loop) {
  ...
  // Flying.fbx ships TWO clips: empty "Take 001" (dur 0, 0 tracks) + real "mixamo.com".
  // animations[0] would bake a frozen statue — always pick the non-empty one.
  const clip = obj.animations.find((a) => a.tracks.length > 0 && a.duration > 0.01) || obj.animations[0];
  console.log(`  ${name}: using clip "${clip.name}" dur=${clip.duration.toFixed(2)}s`);
