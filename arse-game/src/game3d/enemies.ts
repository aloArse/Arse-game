...
export class Enemies {
  ...
  update(ctx: EnemyCtx): void {
    ...
    // Iterate backwards over the live array (NOT a `[...this.list]` copy).
    // this.remove() can splice the current entry out mid-loop (see
    // updateWreck below); going backwards means a splice only shifts
    // indices we've already visited, so it's safe without allocating a
    // brand-new array of every enemy on screen 60 times a second — that
    // spread was a real GC/frame-time cost during big waves and boss
    // fights (exactly when it hurt most).
